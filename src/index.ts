...
export function createService(
  id: string,
  payload: ServiceRenderedPayload
): Result<Saloon, string> {
  ...
  const saloon = match(saloonStorage.get(id), {
    Some: (saloon) => {
      ...
      if (saloon.owner.toString() !== ic.caller().toString()) {
        return Result.Err<Saloon, string>(
          "You are not the owner of this saloon"
        );
      }

      const servicesRendered: Vec<ServiceRendered> = saloon.servicesRendered;
    ...
    None: () =>
      Result.Err<Saloon, string>("Unable to carry out the following function"),
  });
...
export function deleteSaloon(id: string): Result<Saloon, string> {
  return match(saloonStorage.remove(id), {
    ...
    None: () =>
      Result.Err<Saloon, string>(
        `couldn't delete saloon with this id=${id}. saloon not found.`
      ),
  });
}
...
export function rateSaloon(id: string, rate: number): Result<Saloon, string> {
  if (rate < 0 || rate > 5) {
    return Result.Err<Saloon, string>(
      `Error rating saloon with the id=${id}. Invalid rating value.`
    );
  }
  ...
  const saloonRating: any = match(saloonStorage.get(id), {
    ...
    None: () =>
      Result.Err<Saloon, string>(
        `Error updating saloon with the id=${id}. Saloon not found`
      ),
  });
  ...
  return match(saloonStorage.get(id), {
    ...
    None: () =>
      Result.Err<Saloon, string>(
        `Error rating saloon with the id=${id}. Saloon not found`
      ),
  });
}
...
$update;
export function updateSaloonById(
  id: string,
  payload: SaloonPayload
): Result<Saloon, string> {
  if (
    !payload.saloonName ||
    !payload.saloonLocation ||
    ...
    return Result.Err<Saloon, string>(
      "Invalid payload. Payload must contain saloonName, saloonLocation, and attachmentURL."
    );
  }
  return match(saloonStorage.get(id), {
    Some: (saloon) => {
    ...
    None: () =>
      Result.Err<Saloon, string>(
        `couldn't update the saloon with this id=${id}. saloon not found`
      ),
  });
}
